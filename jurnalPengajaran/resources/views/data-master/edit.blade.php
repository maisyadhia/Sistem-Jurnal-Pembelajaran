...
@section('title', 'Edit Data Master - E-Jurnal')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
        <div class="mb-6">
            <h2 class="font-headline-md text-headline-md text-on-background">Edit Data Master</h2>
            <p class="font-body-sm text-body-sm text-on-surface-variant">Perbarui data master yang sudah ada.</p>
        </div>
        
        <form method="POST" action="{{ route('data-master.update', $dataMaster->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('name') border-error @enderror" 
                           id="name" name="name" type="text" required value="{{ old('name', $dataMaster->name) }}"/>
                    @error('name')
                ...
                <div class="space-y-1.5">
                    ...
                    <input class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('identifier') border-error @enderror" 
                           id="identifier" name="identifier" type="text" required value="{{ old('identifier', $dataMaster->identifier) }}"/>
                    @error('identifier')
                        <p class="text-error text-sm">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('initials') border-error @enderror" 
                           id="initials" name="initials" type="text" maxlength="3" required value="{{ old('initials', $dataMaster->initials) }}"/>
                    @error('initials')
                        <p class="text-error text-sm">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('category') border-error @enderror" 
                        ...
                        <option value="GURU TETAP" {{ old('category', $dataMaster->category) == 'GURU TETAP' ? 'selected' : '' }}>Guru Tetap</option>
                        <option value="INFRASTRUKTUR" {{ old('category', $dataMaster->category) == 'INFRASTRUKTUR' ? 'selected' : '' }}>Infrastruktur</option>
                        <option value="MATA PELAJARAN" {{ old('category', $dataMaster->category) == 'MATA PELAJARAN' ? 'selected' : '' }}>Mata Pelajaran</option>
                        <option value="KELAS" {{ old('category', $dataMaster->category) == 'KELAS' ? 'selected' : '' }}>Kelas</option>
                    </select>
                    @error('category')
                        <p class="text-error text-sm">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <select class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('status') border-error @enderror" 
                        ...
                        <option value="Aktif" {{ old('status', $dataMaster->status) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="Tidak Aktif" {{ old('status', $dataMaster->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                        <option value="Ditinjau" {{ old('status', $dataMaster->status) == 'Ditinjau' ? 'selected' : '' }}>Ditinjau</option>
                    </select>
                    @error('status')
                        <p class="text-error text-sm">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-1.5">
                    ...
                    <select class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('color') border-error @enderror" 
                        ...
                        <option value="bg-secondary-container" {{ old('color', $dataMaster->color) == 'bg-secondary-container' ? 'selected' : '' }}>Hijau</option>
                        <option value="bg-primary-container" {{ old('color', $dataMaster->color) == 'bg-primary-container' ? 'selected' : '' }}>Biru</option>
                        <option value="bg-tertiary-container" {{ old('color', $dataMaster->color) == 'bg-tertiary-container' ? 'selected' : '' }}>Oranye</option>
                        <option value="bg-error-container" {{ old('color', $dataMaster->color) == 'bg-error-container' ? 'selected' : '' }}>Merah</option>
                        <option value="bg-primary" {{ old('color', $dataMaster->color) == 'bg-primary' ? 'selected' : '' }}>Biru Tua</option>
                    </select>
                    @error('color')
                        <p class="text-error text-sm">{{ $message }}</p>
                    @enderror
            ...
            <input type="hidden" name="statusColor" value="{{ old('statusColor', $dataMaster->statusColor ?? 'bg-secondary') }}">
