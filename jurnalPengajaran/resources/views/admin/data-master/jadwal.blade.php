                ...
                <tbody class="divide-y divide-outline-variant">
                    @forelse($jadwal as $item)
                        <tr class="hover:bg-surface-container-low transition-colors">
                            <td class="px-4 py-4">{{ $item->hari }}</td>
                            <td class="px-4 py-4">{{ $item->jam_ke }}</td>
                            <td class="px-4 py-4">{{ $item->nama_guru }}</td>
                            <td class="px-4 py-4">{{ $item->nama_kelas }}</td>
                            <td class="px-4 py-4">{{ $item->nama_mapel }}</td>
                            <td class="px-4 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <a href="{{ route('data-master.jadwal.edit', $item->id) }}" 
                                       class="p-1.5 text-on-surface-variant hover:text-primary transition-colors">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </a>
                                    <form method="POST" action="{{ route('data-master.jadwal.destroy', $item->id) }}" 
                                          class="inline-block" 
