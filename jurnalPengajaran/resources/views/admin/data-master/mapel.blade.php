...
<div class="flex flex-wrap gap-2 mb-4">
    <!-- Tombol Export -->
    <a href="{{ route('data-master.export.mapel') }}" 
    ...
    <!-- Form Import -->
    <form method="POST" action="{{ route('data-master.import.mapel') }}" 
...
<div class="bg-surface-container-lowest border border-outline-variant rounded-xl">
    <div class="p-6 border-b border-outline-variant flex justify-between items-center">
        <div>
            <h2 class="font-headline-md text-headline-md text-on-background">Data Mata Pelajaran</h2>
            <p class="font-body-sm text-body-sm text-on-surface-variant">Kelola data mata pelajaran.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-surface-container-low border-b border-outline-variant">
                    <tr>
                        <th class="px-4 py-3 font-label-caps text-label-caps text-on-surface-variant">KODE</th>
                        <th class="px-4 py-3 font-label-caps text-label-caps text-on-surface-variant">NAMA MATA PELAJARAN</th>
                ...
                <tbody class="divide-y divide-outline-variant">
                        ...
                        <tr class="hover:bg-surface-container-low transition-colors">
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-700 font-bold text-[10px]">
                                        {{ strtoupper(substr($item->kode_mapel, 0, 2)) }}
                                    </div>
                                    <span class="font-data-tabular text-data-tabular">{{ $item->kode_mapel }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4">{{ $item->nama_mapel }}</td>
                            <td class="px-4 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <a href="{{ route('data-master.mapel.edit', $item->id) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-8 text-center text-on-surface-variant">
