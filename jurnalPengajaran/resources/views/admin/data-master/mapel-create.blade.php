...
@section('title', 'Tambah Mata Pelajaran - E-Jurnal')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
        <div class="mb-6">
            <h2 class="font-headline-md text-headline-md text-on-background">Tambah Mata Pelajaran</h2>
            <p class="font-body-sm text-body-sm text-on-surface-variant">Tambahkan data mata pelajaran baru.</p>
        </div>
        
        <form method="POST" action="{{ route('data-master.mapel.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('kode_mapel') border-error @enderror" 
                       id="kode_mapel" name="kode_mapel" type="text" required value="{{ old('kode_mapel') }}"/>
                @error('kode_mapel')
            ...
            <div class="space-y-1.5">
                ...
                <input class="w-full h-[40px] bg-surface border border-outline-variant rounded-lg px-4 text-body-base focus:ring-2 focus:ring-secondary focus:border-secondary outline-none transition-all @error('nama_mapel') border-error @enderror" 
                       id="nama_mapel" name="nama_mapel" type="text" required value="{{ old('nama_mapel') }}"/>
                @error('nama_mapel')
                    <p class="text-error text-sm">{{ $message }}</p>
                @enderror
            </div>
