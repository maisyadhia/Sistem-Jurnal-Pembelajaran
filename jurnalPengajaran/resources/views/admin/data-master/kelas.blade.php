...
@section('title', 'Data Kelas - E-Jurnal')
...
<div class="flex flex-wrap gap-2 mb-4">
    <!-- Tombol Export -->
    <a href="{{ route('data-master.export.kelas') }}" 
    ...
    <!-- Form Import -->
    <form method="POST" action="{{ route('data-master.import.kelas') }}" 
...
<div class="bg-surface-container-lowest border border-outline-variant rounded-xl">
    <div class="p-6 border-b border-outline-variant flex justify-between items-center">
        <div>
            <h2 class="font-headline-md text-headline-md text-on-background">Data Kelas</h2>
            <p class="font-body-sm text-body-sm text-on-surface-variant">Kelola data kelas.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-surface-container-low border-b border-outline-variant">
                    <tr>
                        <th class="px-4 py-3 font-label-caps text-label-caps text-on-surface-variant">NAMA KELAS</th>
                        <th class="px-4 py-3 font-label-caps text-label-caps text-on-surface-variant">WALI KELAS</th>
                        ...
                        <tr class="hover:bg-surface-container-low transition-colors">
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-data-tabular text-data-tabular">{{ $item->nama_kelas }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4">{{ $item->wali_kelas }}</td>
                            <td class="px-4 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    <a href="{{ route('data-master.kelas.edit', $item->id) }}" 
                                       class="p-1.5 text-on-surface-variant hover:text-primary transition-colors">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </a>
                                    <button onclick="openDeleteModal('{{ $item->id }}', '{{ $item->nama_kelas }}', 'KELAS')" 
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    ...
    let route = '/admin/data-master/kelas/' + id;
